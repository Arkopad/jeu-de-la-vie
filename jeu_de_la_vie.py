...
class jeu_de_la_vie(Affichage):
    def __init__(self):
        ...
        super().__init__(
            nom=f"Jeu de la vie! (en {self.ligne}x{self.colonne})",
            geometry=(500, 500),
    ...
    def widgets(self):
        ...
        self.titre = tk.Label(
            self.racine,
            text=f"Jeu de la vie! (en {self.ligne}x{self.colonne})",
        ...
        self.police_edition = lambda: (
            "Comic sans Ms",
            int(float((self.width + self.height) / 2) / 40),
            "bold",
        )  # Valeur d'actualisation du label du bouton en fonction de la taille de la fenetre
        self.bouton_edition = tk.Button(
            master=self.buttons_frame,
            text="Mode edition activé",
            font=self.police_edition(),
            background="#141418",
            foreground="white",
            command=self.edition,
        )
        self.bouton_edition.pack(side="left")

        self.police_clear = lambda: (
            "Comic sans Ms",
            int(float((self.width + self.height) / 2) / 40),
            "bold",
        )  # Valeur d'actualisation du label du bouton en fonction de la taille de la fenetre
        self.bouton_clear = tk.Button(
            master=self.buttons_frame,
            text="Clear",
            font=self.police_clear(),
            background="#141418",
            foreground="white",
            command=self.clear,
        )
    ...
    def change_couleur(self, event):
        ...
        if self.mode_edition:
            ...
            messagebox.showerror(
                "Erreur",
                "Vous devez activer le mode edition pour pouvoir modifier la grille",
            )
    ...
    def jeu_de_la_vie(self, grille):
        ...
        grille_suivante = np.array(
            [[0 for j in range(self.colonne)] for i in range(self.ligne)]
        )
